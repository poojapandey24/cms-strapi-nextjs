...
export default function BlogDetailPage({ params }) {
      ...
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          setBlog({
            ...
            image: blogData.image && typeof blogData.image === 'object'
  ...
  return (
    <div className="bg-[#030b1a] text-white min-h-screen">
      <Header />
      <main style={{ padding: "2rem", maxWidth: "900px", margin: "auto", color: "#ffffff" }}>
        {/* Breadcrumb */}
        <nav
        ...
        {blog.image ? (
          <div style={{ margin: "2rem 0" }}>
            <Image
              ...
              width={900}
              height={450}
              style={{ borderRadius: "10px", width: "100%", height: "auto", objectFit: "cover" }}
            />
          </div>
        ) : (
          <div style={{ margin: "2rem 0", width: "100%", height: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f3f3f3', color: '#888', fontSize: '1.1rem', borderRadius: '10px' }}>
            No image available
          </div>
        )}
